...
Two finished deliverables to show attendees what good output looks like:
- **eCommerce — Koala Living** (`ia-map-koalaliving-com-au-*.xlsx`): ~2,956 product PDPs classified, store-locator + collections mapped, and **high-volume category pages flagged as gaps** (Sofas/Lounges 74k, Beds 60k, Mattresses 60k…) — the classic eCommerce IA opportunity.
- **Service — StudioHawk** (`ia-map-studiohawk-com-au-*.xlsx`): services, industries, location pages, case studies and blog mapped, with real SE Ranking volumes on the money pages.

Each example includes the IA Map + Vetting Log (xlsx) and mirrored CSVs.
